...
def user_timeline(username):
    ...
    profile_user = query_database('select * from user where username = ?',
                            [username], one=True)
    if profile_user is None:
        abort(404)
    followed = False
    if g.user:
        followed = query_database('''select 1 from follower where
...
def follow_user(username):
    ...
    if whom_id is None:
        abort(404)
    database = get_database()
    database.execute('insert into follower (who_id, whom_id) values (?, ?)',
...
def unfollow_user(username):
    ...
    if whom_id is None:
        abort(404)
    database = get_database()
    database.execute('delete from follower where who_id=? and whom_id=?',
              [session['user_id'], whom_id])
    database.commit()
